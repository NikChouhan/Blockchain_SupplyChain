...
/********************************************** Owner Section *********************************************/
    ...
    enum roles {norole, supplier, transporter, manufacturer, wholesaler, distributor, pharma, revoke }
...
/********************************************** Supplier Section ******************************************/
    ...
    function getStatusOfRawMaterials(
        bytes32 pid
    ) public
    view
    returns(
...
/********************************************** Transporter Section ***************************************/
    ...
    function loadpacakage(bytes32 pid, uint transportertype, uint receivertype) public {
        ...
        if(transportertype == 1) {  // Supplier to Manufacturer
            ...
            emit ShipmentUpdate(pid, msg.sender, RawProductDetails[pid].receiver,1,1);
            ...
            if(receivertype == 1){
                emit ShipmentUpdate(pid, msg.sender, MedicineBatchDetails[pid].wholesaler,2,1);
            } else if(receivertype == 2){
                emit ShipmentUpdate(pid, msg.sender, MedicineBatchDetails[pid].manufacturer,2,2);
            }
            ...
            emit ShipmentUpdate(pid, msg.sender, MedicineWtoD[pid].receiver,1,1);
            ...
            emit ShipmentUpdate(pid, msg.sender, MedicineDtoR[pid].receiver,1,1);
        }
    }
/********************************************** Manufacturer Section **************************************/
    ...
    function newBatch(
        ...
        MedicineBatchDetails[batchID].rawmaterials = RawMaterials;
        ...
        if(receivertype == 1){
            ...
            MedicineBatchDetails[batchID].distributor = Receiver;
        }

        ManufacturerMedicineBatches[msg.sender].push(batchID);

        emit MedicineNewBatch(batchID, msg.sender, Shipper, Receiver);
    }

    function getMedicineBatchCount() public view returns (uint count){
        require(
            UsersDetails[msg.sender].role == roles.manufacturer,
            "Only manufacturer can call this function"
        );
    ...
    returns(
        ...
        return (
            ...
            MedicineBatchDetails[batchID].rawmaterials,
    ...
    returns(
        ...
        return (
            ...
            MedicineBatchDetails[batchID].distributor,
...
/********************************************** Wholesaler Section ****************************************/
    ...
    function medicineReceived(bytes32 pid) public {
        ...
        if(UsersDetails[msg.sender].role == roles.wholesaler) {
        ...
        }else if(UsersDetails[msg.sender].role == roles.distributor){
            require(
                msg.sender == MedicineBatchDetails[pid].distributor,
            ...
            MedicineBatchAtdistributor[msg.sender].push(pid);
        }
...
/********************************************** distributor Section ***************************************/
    mapping(address => bytes32[]) MedicineBatchAtdistributor;
    ...
    {
        require(
            UsersDetails[msg.sender].role == roles.distributor && msg.sender == MedicineBatchDetails[BatchID].distributor,
    ...
    function getBatchIdCountD() public view returns(uint count){
        ...
        return  MedicineBatchAtdistributor[msg.sender].length;
    }
    ...
    function getBatchIdByIndexD(uint index) public view returns(bytes32 BatchID){
        ...
        return MedicineBatchAtdistributor[msg.sender][index];
    }
/********************************************** Pharma Section ******************************************/
